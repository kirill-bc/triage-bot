...
Replace direct Jira writes with a Jira-native apply path: after triage completes, the service POSTs an outcome payload to a Jira Automation **incoming webhook** rule ("Rule B"), which applies labels, writes the comment, and applies any auto-apply field edits as the Automation actor. Rationale: automation-actor edits don't re-trigger other rules (native loop suppression for §15 re-triage), the service can eventually drop Jira write scopes, apply behavior becomes admin-tunable without deploys, and Automation edits are not constrained by edit-screen/request-type field layouts — eliminating the §13 CLOSM class of label-write 400s. Constraint that forces this shape: Jira Automation "Send web request → Wait for response" has a fixed ~30s timeout, which a triage run (sequential LLM calls + vision + Zendesk) regularly exceeds — so trigger rules stay fire-and-forget and the service calls back. Design rule: the service keeps decisions (labels, comment composition inputs, explicit action directives); Rule B maps `{{webhookData.*}}` into actions and composes comment copy. Plan: `.cursor/plans/jira_automation_callback_delivery_9bfe0933.plan.md`.

- [x] Extract shared outcome decision module (`triage_outcome_rendering.py`: label selection, comment gate/priority signal, auto-apply decision). Comment copy is composed by Rule B from callback `comment` inputs (`post`, `kind`, `topic`, `reason`, `current_priority`); the service does not render ADF/plain-text bodies.
- [x] Add `AutomationWebhookTriageActionExecutor` implementing the `TriageActionExecutor` protocol: versioned JSON payload (`payload_version`, `run_id`, `issue_key`, `project`, `source`, `recommendation`, `labels`, `comment {post, kind, topic, reason, current_priority}`, `actions {apply_bug_to_story, apply_priority {from,to}}`); synchronous POST with `X-Automation-Webhook-Token` header and bounded timeout, **no HTTP retry** (Rule B is not idempotent; a lost 200 plus retry duplicates comments); typed `AutomationCallbackError` on failure (same handler-facing semantics as `JiraActionExecutorError`); no POST on `TriageFailure` / missing issue (mirrors current no-write semantics); returned `TriageActionAppliedFlags` mean **apply requested**, not confirmed — document the analytics semantics shift.
