...
- [ ] Remove the `direct` path (and this flag) once the callback path is proven in production — flag is a migration rollback, not a permanent mode.
- Done when: with `TRIAGE_JIRA_APPLY_MODE=automation_webhook` a real triage run produces labels/comment/field edits applied by the Automation actor and correlated by `run_id`; flipping back to `direct` restores current behavior with no code change; `pytest -m lint`, `mypy .`, and `pytest -m unit` all pass.
...
- [ ] New trigger source (e.g. `priority_changed_retriage`) on `POST /triage` so the handler can distinguish re-triage from first-pass `priority_changed`; new Jira Automation trigger rule fires on priority change **with** `triagebot-reviewed` present (Rule B actor edits are excluded natively by rule-trigger suppression).
