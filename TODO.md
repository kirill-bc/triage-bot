...
- [x] Add `AutomationWebhookTriageActionExecutor` implementing the `TriageActionExecutor` protocol: versioned JSON payload (`payload_version`, `run_id`, `issue_key`, `project`, `source`, `recommendation`, `labels`, `comment {post, body}`, `actions {apply_bug_to_story, apply_priority {from,to}}`); synchronous POST with `X-Automation-Webhook-Token` header, bounded timeout + retry; typed `AutomationCallbackError` on exhaustion (same handler-facing semantics as `JiraActionExecutorError`); no POST on `TriageFailure` / missing issue (mirrors current no-write semantics); returned `TriageActionAppliedFlags` mean **apply requested**, not confirmed — document the analytics semantics shift.
...
- [ ] Add integration tests for `AutomationWebhookTriageActionExecutor` against a mocked Rule B incoming-webhook endpoint: payload contract (`payload_version`, labels, comment, actions), `X-Automation-Webhook-Token` header, retry/timeout behavior, `AutomationCallbackError` mapping, and no-POST on `TriageFailure`.
