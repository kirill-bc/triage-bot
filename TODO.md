...
- [x] Implement `POST /triage` contract accepting `issue_key`, `project`, and `source` (closed `Literal`: `bug_created`, `manual_trigger`, `priority_changed`, `daily_cleanup`, `jira_escalated_added`, `priority_changed_retriage`, `zendesk_ticket_added`).
...
- [x] **Priority-only project profile (CLOSM):** `TRIAGE_PRIORITY_ONLY_PROJECTS` skips classification and runs the existing priority step only (treat intake as Bug; JQL owns filtering). Priority user prompt for those projects reuses the default Bug-path resolution (`triagebot/priority-user` via `TRIAGE_LANGFUSE_PRIORITY_PROMPT_NAME`, then local templates) — no per-project prompt override. Operator: add `CLOSM` to `TRIAGE_ALLOWED_PROJECTS` + `TRIAGE_PRIORITY_ONLY_PROJECTS` and wire a second Jira Automation JQL.
- [ ] **Chained/per-project priority prompts:** a dedicated CLOSM (or other project-specific) priority rubric was deferred; revisit once we have a prompt-chaining approach (with placeholders) rather than a flat Langfuse-name override. Removed `TRIAGE_LANGFUSE_CLOSM_PRIORITY_PROMPT_NAME` and the override plumbing in `prompt_composer`/`triage_handler` until then.
- [x] **Triage date on issue block:** `format_issue_text_block` prefixes `Triage date (UTC): YYYY-MM-DD` (injectable for tests; default UTC today) so priority/Zendesk recency policy can compare dated comments against "today". Local `priority_definition.md` / `priority_template` tell the model to use that date; Langfuse `{{issue_block}}` picks it up automatically.
...
## 12. Concurrency (bounded, no broker)
Investigated OpenRouter behavior under concurrent triage load (debug session 339fc7): no real rate limiting observed, but `httpx`'s read timeout does not bound total call duration because OpenRouter sends keep-alive padding during upstream provider stalls, so a stuck call could run for minutes or hang indefinitely; one reproduction also showed a stalled call eventually returning an empty/unusable HTTP 200 body that a downstream Zendesk-summary catch silently absorbed. Separately, latency on the configured text model degrades roughly 2-4x once ~5-6 calls are in flight concurrently on the same key — a genuine upstream throughput ceiling, not a bug in this service.
- [x] **Hard wall-clock deadline per OpenRouter call:** `TRIAGE_OPENROUTER_CALL_DEADLINE_SECONDS` (default `150`) kills a stalled attempt via a background thread + `Future` timeout and retries once on a fresh connection; the same one-retry policy also covers an unusable (empty/malformed) HTTP 200 body. See `src/triage_service/adapters/openrouter_inference_client.py`.
- [x] **Decision — no message broker:** rejected RabbitMQ/Redis-backed queueing. The measured bottleneck is upstream model throughput degrading past a small concurrency threshold, not request intake; a broker would relocate the wait, not raise the ceiling, while adding persistence/consumer/ops surface to a single-container deployment. Jira Automation's scheduled JQL sweep already re-picks-up untriaged issues, which gives queue-like retry semantics for free.
- [x] **Webhook path bound:** `POST /triage` uses an in-process `threading.Semaphore` sized by `TRIAGE_MAX_CONCURRENT_RUNS` (default `4`, just under the measured degradation knee). A request that cannot acquire a slot within `TRIAGE_CONCURRENCY_WAIT_SECONDS` (default `900`) gets HTTP `503` instead of queuing indefinitely — safe because Jira Automation webhooks here are fire-and-forget and the scheduled sweep retries later. `GET /health` was converted to `async def` so readiness probes are served on the event loop and stay responsive while triage slots are saturated. See `src/triage_service/api/triage_api.py`.
- [x] **Bulk CLI bound:** `triage_bulk_cli.run_bulk_triage` replaced its fully sequential loop with a bounded `ThreadPoolExecutor` (`--concurrency`, default `4`); each worker thread lazily builds and reuses its own `TriageRunner` (own Jira/OpenRouter/Zendesk clients) unless a runner is explicitly injected (tests), matching the pattern already proven safe by `scripts/debug_concurrent_triage_probe.py`. Rows in the JSON report stay in JQL order regardless of completion order; one issue failing does not abort the others. `--concurrency 1` preserves the old sequential behavior.
- [x] Docs: `README.md` *Concurrency* section and `.env.example` for `TRIAGE_MAX_CONCURRENT_RUNS` / `TRIAGE_CONCURRENCY_WAIT_SECONDS`.
- Done when: a burst of concurrent `POST /triage` calls is bounded and sheds excess load with `503` instead of hanging or silently degrading; bulk triage runs issues in parallel up to a configurable cap without cross-thread state sharing; `pytest -m lint`, `mypy .`, and `pytest -m unit` all pass.

## 13. Reliability: CLOSM label-write failure fallout (2026-09-04)
First production triage of a CLOSM ticket (`CLOSM-39487`) completed classification/priority inference and Langfuse tracing correctly, but the `triagebot-reviewed` label write failed: Jira returned HTTP 400 (`Field 'labels' cannot be set. It is not on the appropriate screen, or unknown.`). That `JiraActionExecutorError` was then masked by a second, unrelated bug — a `RuntimeError: generator didn't stop after throw()` from the Langfuse tracing context manager — so the run surfaced as an opaque 500 even though `triage_completed` had already been recorded to the audit store. Live investigation (`editmeta`, `mypermissions`, `project` on the real issue/project) confirmed: triagebot has `EDIT_ISSUES` + `BROWSE_PROJECTS` on the issue (not an account/permission-scope problem); `editmeta.fields` is completely empty for this issue (not just missing `labels`); CLOSM is a classic (company-managed) Jira Service Management project (`projectTypeKey: service_desk`, `style: classic`) with issue type `[System] Service request`. Most likely cause: the request type's field layout / classic Edit screen for that issue type doesn't include `Labels` — a Jira admin-side config gap, not a code bug — but it also exposed a real exception-masking bug in the Langfuse tracing context manager.

- [x] **Jira admin action (ops, not code):** add `Labels` to the CLOSM request type's field layout (*Project settings → Request types → [type behind `[System] Service request`] → Fields*) and confirm the underlying issue type's classic Edit screen also includes `Labels`. Re-verify via `GET /rest/api/3/issue/{key}/editmeta` that `labels` appears in the response before re-running triage on a CLOSM ticket.
- [x] **Fix Langfuse tracing context-manager bug (masks the real exception):** `LangfuseInferenceTracer.triage_run_session` and `.triage_issue_trace` (`src/triage_service/observability/langfuse_inference_tracing.py`) catch `Exception` around the inner `yield` and then `yield` again — invalid once `contextlib` has called `gen.throw()` on a `@contextmanager` generator, which raises `RuntimeError: generator didn't stop after throw()` and replaces the real underlying error (e.g. `JiraActionExecutorError`) for **any** exception that escapes the `with` block, not just Langfuse-specific failures. TDD: add a test that raises inside `with tracer.triage_run_session(...)` / `.triage_issue_trace(...)` and asserts the original exception propagates unchanged (no `RuntimeError` about generators). Fix by not re-yielding after catching the thrown exception (log, then let the original exception propagate — no second `yield`).
- Done when: `labels` is present in CLOSM `editmeta`; no exception escaping the triage pipeline is ever reported as `RuntimeError: generator didn't stop after throw()`, with a regression test covering the tracer double-yield bug; `pytest -m lint`, `mypy .`, and `pytest -m unit` all pass.

## 14. Jira Automation callback delivery (Phase A — active)
Replace direct Jira writes with a Jira-native apply path: after triage completes, the service POSTs a fully-rendered outcome payload to a Jira Automation **incoming webhook** rule ("Rule B"), which applies labels, the comment, and any auto-apply field edits as the Automation actor. Rationale: automation-actor edits don't re-trigger other rules (native loop suppression for §15 re-triage), the service can eventually drop Jira write scopes, apply behavior becomes admin-tunable without deploys, and Automation edits are not constrained by edit-screen/request-type field layouts — eliminating the §13 CLOSM class of label-write 400s. Constraint that forces this shape: Jira Automation "Send web request → Wait for response" has a fixed ~30s timeout, which a triage run (sequential LLM calls + vision + Zendesk) regularly exceeds — so trigger rules stay fire-and-forget and the service calls back. Design rule: the service keeps all decisions and copy (final label list, final comment text, explicit action directives); Rule B is a dumb applier mapping `{{webhookData.*}}` into actions. Plan: `.cursor/plans/jira_automation_callback_delivery_9bfe0933.plan.md`.

- [x] Extract shared outcome decision/rendering module from `jira_action_executor.py` (label selection, comment gate/priority signal, auto-apply decision, comment rendering from `jira_comment_templates.json` in ADF **and** plain-text — automation path renders reporter mention as `[~accountid:...]`); `JiraTriageActionExecutor` consumes it with no behavior change (tests first, existing executor tests stay green).
- [x] Add `AutomationWebhookTriageActionExecutor` implementing the `TriageActionExecutor` protocol: versioned JSON payload (`payload_version`, `run_id`, `issue_key`, `project`, `source`, `recommendation`, `labels`, `comment {post, body}`, `actions {apply_bug_to_story, apply_priority {from,to}}`); synchronous POST with `X-Automation-Webhook-Token` header and bounded timeout, **no HTTP retry** (Rule B is not idempotent; a lost 200 plus retry duplicates comments); typed `AutomationCallbackError` on failure (same handler-facing semantics as `JiraActionExecutorError`); no POST on `TriageFailure` / missing issue (mirrors current no-write semantics); returned `TriageActionAppliedFlags` mean **apply requested**, not confirmed — document the analytics semantics shift.
- [x] Settings + wiring: `TRIAGE_JIRA_APPLY_MODE` = `direct` (default, rollback path) | `automation_webhook`; authenticated `POST /triage` may select either mode per request with optional `jira_apply_mode` (omission uses the env default, enabling hybrid migration without process-env mutation); `JIRA_AUTOMATION_WEBHOOK_URL` (required before a webhook-mode request starts inference), `JIRA_AUTOMATION_WEBHOOK_TOKEN`, `JIRA_AUTOMATION_WEBHOOK_TIMEOUT_SECONDS`; executor selection in `build_default_triage_handler` (`apply_to_jira=False` → NoOp unchanged).
- [x] Observability: new `outcome_delivered` audit event (mode, HTTP status, attempt count) in the `TriageAuditEvent` union, recorded on success and failure; `run_id` in the payload for correlating Rule B audit-log entries back to traces.
- [x] Docs/ops: `docs/ops/jira_automation_callback.md` — Rule B construction (incoming-webhook trigger with `issue_key`, token, actions mapped from `{{webhookData.labels}}` / `{{webhookData.comment.body}}` / conditional branches on `{{webhookData.actions.*}}`, Log action for payload debugging), operator smoke checklist (manual triage lands labels/comment/edits; Rule B edits do NOT re-fire the priority-change trigger rule), operator flow for diagnosing Rule B-side apply failures (rule audit log lookup by `run_id`, distinguishing "triage succeeded, apply failed" from "triage failed" via `outcome_delivered`), caveats (async `triagebot-reviewed` dedupe-marker latency vs the scheduled-scan JQL window, including the inference cost bound when a callback never lands and the issue keeps re-matching the `created >= -30m` window; multi-project rule execution quotas). README feature-flag section incl. ADF-vs-`[~accountid:...]` mention difference.
- [x] Remove the `direct` path (and this flag) once the callback path is proven in production — flag is a migration rollback, not a permanent mode.
- Done when: with the Automation callback a real triage run produces labels/comment/field edits applied by the Automation actor and correlated by `run_id`; `TRIAGE_JIRA_APPLY_MODE` / `jira_apply_mode` / `JiraTriageActionExecutor` are gone; leftover env/body values are ignored; `pytest -m lint`, `mypy .`, and `pytest -m unit` all pass.

## 15. Re-triage on priority change (Phase B+C — after §14)
Extend triage triggers so a priority change on an **already-triaged** ticket re-runs triage, gated so it only happens after a meaningful change to the ticket (description edit, new human comments, Zendesk context change), with prompt-side handling of the bot's own prior comments. Depends on §14: Rule B's automation-actor edits give native loop suppression, and Rule B's *Set entity property* action gives Jira-native last-run state.

Prompt updates (C):
- [ ] Add a `## Prior TriageBot assessments (re-triage)` section to Langfuse `triagebot/classification-user` and `triagebot/priority-user`, placed directly after `## Human override`: prior TriageBot comments (recognizable by the fixed intro from `jira_comment_templates.json`) are the bot's own previous run — re-derive from current state and policy; never cite prior TriageBot rationale as evidence (no self-reinforcement); a human reply that answers a prior TriageBot comment with policy-engaging reasoning counts under the Human override (VIP guard still overrides); a priority change with no stated reasoning is not an override; if the recommendation is unchanged with no material change, say so in one sentence instead of re-arguing, and if it changed, name what changed; prior TriageBot comments never satisfy the P2→P1 / P3→P2 aging clauses' "client asked for updates" condition.
- [ ] Mirror the clause into the local fallback (`reason_for_humans` in `src/triage_service/core/prompt_templates.json`) so the non-Langfuse path stays in sync.
- [ ] Unit test asserting the prior TriageBot comment and a human reply survive `TRIAGE_COMMENTS_CHAR_BUDGET` selection into the composed issue block for the re-triage path (budget keeps a newest-comment suffix; guard against future budget tightening silently dropping the clause's inputs).

Enforcement / gating (B — deterministic, service- and Jira-side; no LLM call to decide whether to run):
- [x] Add `priority_changed_retriage` to `POST /triage` so the handler can distinguish re-triage from first-pass `priority_changed` (included with the full seven-source API/audit contract).
- [ ] New Jira Automation trigger rule fires on priority change **with** `triagebot-reviewed` present (Rule B actor edits are excluded natively by rule-trigger suppression).
- [ ] Last-run state as a Jira issue **entity property** set by Rule B from the §14 payload (`run_id`, timestamp, recommendation, fingerprint) — service computes the fingerprint (hash over summary, description, reproduction steps, linked Zendesk ticket ids + last-activity timestamps) and ships it in the callback payload; service reads the property back (read-only) during the gate.
- [ ] Gate in `triage_handler` after Jira fetch, before any inference, re-triage source only: skip when the new priority equals the stored last recommendation (someone just applied the bot's advice); skip when fingerprint is unchanged AND no new non-bot comments since last run; per-issue cooldown (e.g. 24h) regardless of changes; Rule A-side condition on `{{issue.properties...}}` for the applied-recommendation guard where expressible natively.
- [ ] Audit: `triage_skipped_no_material_change` (or equivalent) event with skip reason so gated runs are observable in Langfuse/analytics.
- [ ] Decide comment-noise policy on repeat mismatch: suppress duplicate advisory comments when the recommendation is unchanged (labels only), or post at most once per N days — compare against stored last recommendation.
- Done when: a priority change on a triaged ticket with no material change produces an audited skip and no LLM spend; a material change (description edit / new human comment / Zendesk activity) produces a full re-triage whose reason references what changed; a justified human reply to a prior TriageBot comment is honored as a human override in the new run; no automation loop occurs (bot-applied edits never re-trigger); all gates pass.

## 16. Integration tests (deferred until post-deploy stabilization)
Assumes the §14 end state: the service does not write to Jira directly; outcomes are delivered as a callback payload to the Rule B incoming webhook.
...
- [ ] Add integration tests for `AutomationWebhookTriageActionExecutor` against a mocked Rule B incoming-webhook endpoint: payload contract (`payload_version`, labels, comment, actions), `X-Automation-Webhook-Token` header, single-attempt timeout (no HTTP retry), `AutomationCallbackError` mapping, and no-POST on `TriageFailure`.
- [ ] Add integration tests for policy retrieval adapter using mocked content source.
- [ ] Add integration tests for mismatch/no-mismatch behavior and sequential flow (Story outcome skips priority inference; Bug outcome invokes it) — label combinations and comment/action directives asserted on the outgoing callback payload, not on Jira REST calls.
- [ ] Add integration coverage for audit emission on success/failure paths (including model-output capture, confidence persistence, and `outcome_delivered` success/failure).
