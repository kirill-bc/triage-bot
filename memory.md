...
- **Phase close (`/close-phase`):** From `.venv`, `pytest -m lint`, `mypy .` (81 files), and `pytest -m "unit or integration"` all passed (**361 passed**, **1 skipped** `OPENROUTER_LIVE_SMOKE`, **5 deselected**). **§7 image context extraction** shipped end-to-end: ADF inline attachment discovery (`issue_text_block`, `jira_issue_fetcher` + `renderedFields` URL matching), `ImageContextExtractor` / `OpenRouterVisionImageContextExtractor`, vision Langfuse prompts (`vision_prompt_composer`), handler wiring (`run_sync` preprocess; `run_sync_on_fetched` accepts pre-extracted contexts), observability (`image_context_extracted`, `image_context_extraction` span, vision generation redaction in `payload_redaction.sanitize_vision_messages`), benchmark stratification (`has_images` / `text_only`), CLI parity (`image_context` in JSON), smoke script `scripts/fetch_jira_issue_image_context.py`. Feature-flagged off by default (`TRIAGE_IMAGE_CONTEXT_ENABLED`). README / `.env.example` / `TODO.md` §7 reconciled.
...
- **Image context CLI parity (§7):** `scripts/run_triage_cli.py` / `triage_manual_cli.main()` builds the default handler once, runs triage with `TRIAGE_IMAGE_CONTEXT_ENABLED` wiring unchanged, and prints JSON including `image_context` from `build_cli_image_context_summary()` (compact per-attachment rows; no transcripts). `TriageHandler.last_image_extraction` stores the latest `ImageContextExtractionResult` from `run_sync` / `run_sync_on_fetched`. Tests: `test_triage_manual_cli.py`, `test_image_context_wiring.py` (last extraction assert). README documents flags, cost/PII, example `issue_block` excerpt, and CLI output shape.
